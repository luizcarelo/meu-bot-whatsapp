...
"""
Etapa 22 - Aplicar visual compartilhado em views/super-admin.ejs

Objetivo:
- Criar backup antes da alteracao.
- Gerar manifesto antes e depois.
- Alterar somente views/super-admin.ejs.
- Garantir link para /css/style.css.
- Injetar camada visual controlada com marcadores ETAPA22.
- Preservar logica existente, fetch, botoes, modais e formularios.
- Nao alterar backend.
- Nao alterar banco.
- Reiniciar app somente se ETAPA22_RESTART_APP=true.
- Validar login, dashboard e /super-admin.
- Atualizar documentacao obrigatoria.
- Gerar relatorios em reports.

Como executar:
sudo ETAPA22_RESTART_APP=true ETAPA22_LOGIN_EMAIL='admin@saas.com' ETAPA22_LOGIN_PASSWORD='123456' python3 etapa_22_aplicar_visual_super_admin.py
"""
...
VIEW_PATH = ROOT / "views" / "super-admin.ejs"
...
def bloco_visual_super_admin():
    return """
<!-- ETAPA22_SUPER_ADMIN_VISUAL_INICIO -->
<script id="etapa22-super-admin-visual">
(function () {
    function existe(id) {
    ...
    function criarCabecalho() {
        if (existe('etapa22-super-admin-header')) {
            return;
        }
        ...
        header.id = 'etapa22-super-admin-header';
        header.className = 'er-page-header';
        header.innerHTML = ''
            + '<div class="er-top-actions" style="justify-content: space-between;">'
            + '  <div>'
            + '    <div class="er-badge er-badge-success">Super Admin</div>'
            + '    <h1 class="er-page-title" style="margin-top: 12px;">Gestao geral da plataforma</h1>'
            + '    <p class="er-page-subtitle">Empresas, administradores, limites e operacao central em uma interface padronizada.</p>'
            + '  </div>'
            + '  <div class="er-top-actions">'
            + '    <span class="er-badge er-badge-muted">Visual Etapa 22</span>'
    ...
    function aplicarClasses() {
        ...
        for (var i = 0; i < cards.length; i++) {
            if (cards[i].id === 'etapa22-super-admin-header') {
                continue;
            }
...
<!-- ETAPA22_SUPER_ADMIN_VISUAL_FIM -->
"""
...
def aplicar_visual():
    resultado = {
        "arquivo": "views/super-admin.ejs",
        "existe_antes": VIEW_PATH.exists(),
        "alterado": False,
        "adicionou_css": False,
        "adicionou_script": False,
        "sha256_antes": sha256(VIEW_PATH) if VIEW_PATH.exists() else None,
        "sha256_depois": None
    }

    texto = ler(VIEW_PATH)
    if texto is None:
        resultado["erro"] = "views/super-admin.ejs ausente ou ilegivel"
        return resultado
    ...
    if "ETAPA22_SUPER_ADMIN_VISUAL_INICIO" not in novo:
        novo = inserir_antes_de_body_fim(novo, bloco_visual_super_admin())
        resultado["adicionou_script"] = True

    if novo != texto:
        gravar(VIEW_PATH, novo)
        resultado["alterado"] = True

    resultado["sha256_depois"] = sha256(VIEW_PATH)
    return resultado
...
def validar_estrutura():
    ...
    resultado = {
        "arquivo_existe": VIEW_PATH.exists(),
    ...
    resultado["tem_marker"] = "ETAPA22_SUPER_ADMIN_VISUAL_INICIO" in texto
    resultado["tem_fetch"] = "fetch(" in texto
    resultado["tem_texto_visual"] = "Super Admin" in texto and "Gestao geral da plataforma" in texto
...
def restart_app():
    valor = os.environ.get("ETAPA22_RESTART_APP", "").strip().lower()
...
def http_request(opener, metodo, path, data_obj=None, timeout=15):
    ...
    body_bytes = None
    headers = {
        "User-Agent": "etapa-22-visual-super-admin/1.0"
    }
...
def atualizar_documentacao(relatorio):
    ...
    melhoria = relatorio["melhoria_super_admin"]
...
def gerar_md(relatorio):
    ...
    linhas = []
    linhas.append("# Etapa 22 - Aplicar visual compartilhado em Super Admin")
    linhas.append("")
    ...
    linhas.append("- views/super-admin.ejs alterado: " + str(melhoria["alterado"]))
...
def main():
    ...
    stamp = agora_stamp()
    backup_dir = BACKUPS_DIR / ("etapa_22_" + stamp)

    manifesto_antes = gerar_manifesto()
    manifesto_antes_path = REPORTS_DIR / "etapa_22_manifesto_antes.json"
    salvar_json(manifesto_antes_path, manifesto_antes)

    backup = criar_backup(backup_dir)

    since = logs_since()
    melhoria = aplicar_visual()
    ...
    manifesto_depois_path = REPORTS_DIR / "etapa_22_manifesto_depois.json"
    salvar_json(manifesto_depois_path, manifesto_depois)

    relatorio["manifesto_depois"] = rel(manifesto_depois_path)

    json_path = REPORTS_DIR / "etapa_22_aplicar_visual_super_admin.json"
    md_path = REPORTS_DIR / "etapa_22_aplicar_visual_super_admin.md"

    salvar_json(json_path, relatorio)
    gravar(md_path, gerar_md(relatorio))

    print("Etapa 22 concluida.")
    ...
    print("views/super-admin.ejs alterado: " + str(melhoria["alterado"]))
