...
"""
Etapa 23.2 - Forcar /super-admin a servir views/super-admin.ejs via sendFile

Objetivo:
- Criar backup antes da alteracao.
- Alterar somente routes/index.js e documentacao obrigatoria.
- Garantir require path.
- Trocar res.render('super-admin') por res.type('html').sendFile(...).
- Rodar node --check routes/index.js.
- Reiniciar app.
- Validar login, dashboard e /super-admin.
- Validar que /super-admin contem os marcadores da Etapa 22.
- Gerar relatorios JSON e Markdown.

Como executar:
sudo ETAPA23_2_LOGIN_EMAIL='admin@saas.com' ETAPA23_2_LOGIN_PASSWORD='123456' python3 etapa_23_2_forcar_super_admin_sendfile.py
"""
...
def aplicar_correcao():
    ...
    linha_path = "const path = require('path');"
    if linha_path not in novo:
        alvo = "const express = require('express');"
        pos = novo.find(alvo)

        if pos >= 0:
            fim = novo.find("\n", pos)
            if fim >= 0:
                novo = novo[:fim + 1] + linha_path + "\n" + novo[fim + 1:]
                resultado["adicionou_path"] = True
        else:
            novo = linha_path + "\n" + novo
            resultado["adicionou_path"] = True

    antigo_1 = "return res.render('super-admin');"
    antigo_2 = 'return res.render("super-admin");'
    novo_retorno = "return res.type('html').sendFile(path.join(__dirname, '..', 'views', 'super-admin.ejs'));"

    if antigo_1 in novo:
        novo = novo.replace(antigo_1, novo_retorno, 1)
        resultado["trocou_render"] = True
    elif antigo_2 in novo:
        novo = novo.replace(antigo_2, novo_retorno, 1)
        resultado["trocou_render"] = True
    elif novo_retorno in novo:
        resultado["trocou_render"] = False
    else:
        resultado["erro"] = "Nao encontrei res.render('super-admin') para substituir"
...
def validar_estrutura():
    ...
    resultado["tem_path"] = "const path = require('path');" in texto
    resultado["tem_rota_super_admin"] = "router.get('/super-admin'" in texto or 'router.get("/super-admin"' in texto
    resultado["tem_sendfile"] = "sendFile(path.join(__dirname, '..', 'views', 'super-admin.ejs'))" in texto
    resultado["tem_type_html"] = "res.type('html')" in texto or 'res.type("html")' in texto
    ...
    resultado["ok"] = bool(
        ...
        resultado["tem_path"] and
        resultado["tem_rota_super_admin"] and
        resultado["tem_sendfile"] and
        resultado["tem_type_html"] and
...
def http_request(opener, metodo, path, data_obj=None, timeout=20, limite=600000):
    url = BASE_URL + path
    ...
    body_bytes = None
    headers = {
        "User-Agent": "etapa-23-2-super-admin-sendfile/1.0"
    }
...
def validar_runtime():
    ...
    super_admin = http_request(opener, "GET", SUPER_ADMIN_PAGE, None, limite=600000)
...
def atualizar_documentacao(relatorio):
    ...
    atualizar_doc(
        ...
        [
            ...
            "Require path adicionado: " + str(correcao["adicionou_path"]) + ".",
            "Troca de render aplicada: " + str(correcao["trocou_render"]) + ".",
...
def gerar_md(relatorio):
    ...
    linhas = []
    linhas.append("# Etapa 23.2 - Super Admin via sendFile")
    linhas.append("")
    ...
    linhas.append("- Adicionou path: " + str(correcao["adicionou_path"]))
    linhas.append("- Trocou render: " + str(correcao["trocou_render"]))
...
def main():
    ...
    stamp = agora_stamp()
    backup_dir = BACKUPS_DIR / ("etapa_23_2_super_admin_sendfile_" + stamp)
    ...
    json_path = REPORTS_DIR / "etapa_23_2_forcar_super_admin_sendfile.json"
    md_path = REPORTS_DIR / "etapa_23_2_forcar_super_admin_sendfile.md"

    salvar_json(json_path, relatorio)
    gravar(md_path, gerar_md(relatorio))

    print("Etapa 23.2 concluida.")
    ...
    print("Adicionou path: " + str(correcao["adicionou_path"]))
    print("Trocou render: " + str(correcao["trocou_render"]))
