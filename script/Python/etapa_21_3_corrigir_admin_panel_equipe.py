...
"""
Etapa 21.3 - Corrigir consulta de equipe no AdminPanelController

Objetivo:
- Criar backup antes da alteracao.
- Gerar manifesto antes e depois.
- Alterar somente controllers/AdminPanelController.js.
- Remover a coluna telefone da consulta de equipe.
- Rodar node --check em controllers/AdminPanelController.js.
- Reiniciar app somente se ETAPA21_3_RESTART_APP=true.
- Validar login, dashboard e /admin/painel.
- Atualizar documentacao obrigatoria.
- Gerar relatorios em reports.

Como executar:
sudo ETAPA21_3_RESTART_APP=true ETAPA21_3_LOGIN_EMAIL='admin@saas.com' ETAPA21_3_LOGIN_PASSWORD='123456' python3 etapa_21_3_corrigir_admin_panel_equipe.py
"""
...
def corrigir_controller():
    resultado = {
        ...
        "alterado": False,
        "consulta_antiga_encontrada": False,
    ...
    antigo = "SELECT id, nome, email, is_admin, cargo, ativo, telefone FROM usuarios_painel WHERE empresa_id = ? ORDER BY nome ASC"
    novo_sql = "SELECT id, nome, email, is_admin, cargo, ativo FROM usuarios_painel WHERE empresa_id = ? ORDER BY nome ASC"

    novo = texto

    if antigo in novo:
        resultado["consulta_antiga_encontrada"] = True
        novo = novo.replace(antigo, novo_sql, 1)
    ...
    resultado["consulta_corrigida_presente"] = novo_sql in depois
...
def validar_estrutura():
    ...
    resultado = {
        ...
        "tem_consulta_equipe_corrigida": False,
        "nao_tem_telefone_na_consulta_equipe": False,
    ...
    consulta_corrigida = "SELECT id, nome, email, is_admin, cargo, ativo FROM usuarios_painel WHERE empresa_id = ? ORDER BY nome ASC"
    consulta_antiga = "SELECT id, nome, email, is_admin, cargo, ativo, telefone FROM usuarios_painel WHERE empresa_id = ? ORDER BY nome ASC"

    resultado["tem_render_panel"] = "async renderPanel" in texto
    resultado["tem_consulta_equipe_corrigida"] = consulta_corrigida in texto
    resultado["nao_tem_telefone_na_consulta_equipe"] = consulta_antiga not in texto
    ...
    resultado["ok"] = bool(
        ...
        resultado["tem_consulta_equipe_corrigida"] and
        resultado["nao_tem_telefone_na_consulta_equipe"] and
...
def restart_app():
    valor = os.environ.get("ETAPA21_3_RESTART_APP", "").strip().lower()
...
def http_request(opener, metodo, path, data_obj=None, timeout=15):
    ...
    body_bytes = None
    headers = {
        "User-Agent": "etapa-21-3-admin-panel-equipe/1.0"
    }
...
def atualizar_documentacao(relatorio):
    ...
    atualizar_doc(
        ...
        [
            ...
            "Consulta antiga encontrada: " + str(correcao["consulta_antiga_encontrada"]) + ".",
...
def gerar_md(relatorio):
    ...
    linhas = []
    linhas.append("# Etapa 21.3 - Corrigir equipe AdminPanelController")
    linhas.append("")
    ...
    linhas.append("- Consulta antiga encontrada: " + str(correcao["consulta_antiga_encontrada"]))
...
def main():
    ...
    stamp = agora_stamp()
    backup_dir = BACKUPS_DIR / ("etapa_21_3_" + stamp)

    manifesto_antes = gerar_manifesto()
    manifesto_antes_path = REPORTS_DIR / "etapa_21_3_manifesto_antes.json"
    ...
    manifesto_depois_path = REPORTS_DIR / "etapa_21_3_manifesto_depois.json"
    salvar_json(manifesto_depois_path, manifesto_depois)

    relatorio["manifesto_depois"] = rel(manifesto_depois_path)

    json_path = REPORTS_DIR / "etapa_21_3_corrigir_admin_panel_equipe.json"
    md_path = REPORTS_DIR / "etapa_21_3_corrigir_admin_panel_equipe.md"

    salvar_json(json_path, relatorio)
    gravar(md_path, gerar_md(relatorio))

    print("Etapa 21.3 concluida.")
