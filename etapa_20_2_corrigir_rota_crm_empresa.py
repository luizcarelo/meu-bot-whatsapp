...
- Corrigir consulta da rota /crm para SELECT * FROM empresas.
- Rodar node --check em routes/index.js.
- Reiniciar app somente se ETAPA20_2_RESTART_APP=true.
- Validar login, dashboard e /crm.
- Atualizar documentacao obrigatoria.
- Gerar relatorios em reports.

Como executar sem restart:
sudo ETAPA20_2_LOGIN_EMAIL='admin@saas.com' ETAPA20_2_LOGIN_PASSWORD='123456' python3 etapa_20_2_corrigir_rota_crm_empresa.py

Como executar com restart:
sudo ETAPA20_2_RESTART_APP=true ETAPA20_2_LOGIN_EMAIL='admin@saas.com' ETAPA20_2_LOGIN_PASSWORD='123456' python3 etapa_20_2_corrigir_rota_crm_empresa.py
"""
...
def corrigir_consulta_crm():
    resultado = {
        "arquivo": "routes/index.js",
        "existe_antes": ROUTES_INDEX.exists(),
        "alterado": False,
        "encontrou_consulta_antiga": False,
    ...
    antigo = """const result = await db.query(
            'SELECT id, nome, logo, cor, plano, limite_usuarios FROM empresas WHERE id = ? LIMIT 1',
            [empresaId]
        );"""

    novo_bloco = """const result = await db.query(
            'SELECT * FROM empresas WHERE id = ? LIMIT 1',
            [empresaId]
        );"""

    validar_sem_asterisco(novo_bloco, "consulta crm nova")

    novo = texto

    if antigo in texto:
        resultado["encontrou_consulta_antiga"] = True
        novo = texto.replace(antigo, novo_bloco, 1)
    else:
        alvo_sql = "'SELECT id, nome, logo, cor, plano, limite_usuarios FROM empresas WHERE id = ? LIMIT 1'"
        if alvo_sql in texto:
            resultado["encontrou_consulta_antiga"] = True
            novo = texto.replace(alvo_sql, "'SELECT * FROM empresas WHERE id = ? LIMIT 1'", 1)

    if novo != texto:
        gravar_texto(ROUTES_INDEX, novo)
        resultado["alterado"] = True

    texto_depois = ler_texto(ROUTES_INDEX) or ""
    resultado["consulta_corrigida_presente"] = "'SELECT * FROM empresas WHERE id = ? LIMIT 1'" in texto_depois
...
def validar_estrutura():
    ...
    resultado = {
        ...
        "nao_tem_logo_na_consulta": False,
    ...
    resultado["tem_select_star"] = "'SELECT * FROM empresas WHERE id = ? LIMIT 1'" in texto or '"SELECT * FROM empresas WHERE id = ? LIMIT 1"' in texto
    resultado["nao_tem_logo_na_consulta"] = "SELECT id, nome, logo, cor, plano, limite_usuarios FROM empresas" not in texto
    ...
    resultado["ok"] = bool(
        ...
        resultado["tem_select_star"] and
        resultado["nao_tem_logo_na_consulta"] and
        resultado["tem_render_crm"]
    )
...
def reiniciar_app_se_solicitado():
    valor = os.environ.get("ETAPA20_2_RESTART_APP", "").strip().lower()
...
def http_request(opener, metodo, path, data_obj=None, timeout=15):
    ...
    body_bytes = None
    headers = {
        "User-Agent": "etapa-20-2-rota-crm-empresa/1.0"
    }
...
def acrescentar_secao_documento(nome, titulo, corpo):
    ...
    marcador_inicio = "<!-- ETAPA_20_2_INICIO -->"
    marcador_fim = "<!-- ETAPA_20_2_FIM -->"
...
def atualizar_documentacao(relatorio):
    ...
    correcao = relatorio["correcao_consulta"]
    ...
    acrescentar_secao_documento(
        ...
        [
            ...
            "Consulta antiga encontrada: " + str(correcao["encontrou_consulta_antiga"]) + ".",
...
def gerar_markdown_relatorio(relatorio):
    ...
    linhas.append("# Etapa 20.2 - Corrigir consulta da rota CRM")
    linhas.append("")
    ...
    linhas.append("- routes/index.js alterado: " + str(correcao["alterado"]))
    linhas.append("- Consulta antiga encontrada: " + str(correcao["encontrou_consulta_antiga"]))
    ...
    linhas.append("")
    linhas.append("- Arquivo: " + correcao["arquivo"])
    linhas.append("- SHA256 antes: " + str(correcao["sha256_antes"]))
    linhas.append("- SHA256 depois: " + str(correcao["sha256_depois"]))
    ...
    linhas.append("- A rota /crm continua protegida por isAuthenticated.")
    linhas.append("- O app so foi reiniciado se ETAPA20_2_RESTART_APP=true.")
...
def main():
    ...
    stamp = agora_stamp()
    backup_dir = BACKUPS_DIR / ("etapa_20_2_" + stamp)

    manifesto_antes = gerar_manifesto()
    manifesto_antes_path = REPORTS_DIR / "etapa_20_2_manifesto_antes.json"
    salvar_json(manifesto_antes_path, manifesto_antes)

    backup = criar_backup(backup_dir)

    correcao = corrigir_consulta_crm()
    ...
    manifesto_depois_path = REPORTS_DIR / "etapa_20_2_manifesto_depois.json"
    salvar_json(manifesto_depois_path, manifesto_depois)

    relatorio["manifesto_depois"] = rel(manifesto_depois_path)

    json_path = REPORTS_DIR / "etapa_20_2_corrigir_rota_crm_empresa.json"
    md_path = REPORTS_DIR / "etapa_20_2_corrigir_rota_crm_empresa.md"

    salvar_json(json_path, relatorio)
    gravar_texto(md_path, gerar_markdown_relatorio(relatorio))

    print("Etapa 20.2 concluida.")
    ...
    print("routes/index.js alterado: " + str(correcao["alterado"]))
    print("Consulta antiga encontrada: " + str(correcao["encontrou_consulta_antiga"]))
    ...
    if not restart["executado"]:
        print("")
        print("Aviso: app nao foi reiniciado. Para aplicar em runtime, execute com ETAPA20_2_RESTART_APP=true.")
