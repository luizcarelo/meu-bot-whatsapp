...
- Corrigir funcao legada IFNULL/VALUES no upsert de contatos.
...
def corrigir_session_manager():
    ...
    for termo in ["IFNULL", "VALUES(foto_perfil)"]:
        resultado["ocorrencias_antes"][termo] = contar_ocorrencias(texto, termo)

    resultado["contexto_antes"] = trecho_contexto(texto, "IFNULL")

    antigo = "foto_perfil = IFNULL(VALUES(foto_perfil), foto_perfil),"
    novo_valor = "foto_perfil = COALESCE(EXCLUDED.foto_perfil, contatos.foto_perfil),"
    ...
    for termo in ["IFNULL", "VALUES(foto_perfil)"]:
        resultado["ocorrencias_depois"][termo] = contar_ocorrencias(texto_depois, termo)

    resultado["contexto_depois"] = trecho_contexto(texto_depois, "COALESCE(EXCLUDED.foto_perfil")
...
def acrescentar_secao_documento(nome, titulo, corpo):
    ...
    marcador_inicio = "<!-- ETAPA_06_2_INICIO -->"
    marcador_fim = "<!-- ETAPA_06_2_FIM -->"
...
def gerar_markdown_relatorio(relatorio):
    ...
    linhas.append("# Etapa 06.2 - Corrigir funcao legada no upsert PostgreSQL")
    linhas.append("")
...
def main():
    ...
    stamp = agora_stamp()
    backup_dir = BACKUPS_DIR / ("etapa_06_2_" + stamp)

    manifesto_antes = gerar_manifesto()
    manifesto_antes_path = REPORTS_DIR / "etapa_06_2_manifesto_antes.json"
    ...
    manifesto_depois_path = REPORTS_DIR / "etapa_06_2_manifesto_depois.json"
    salvar_json(manifesto_depois_path, manifesto_depois)

    relatorio["manifesto_depois"] = rel(manifesto_depois_path)

    json_path = REPORTS_DIR / "etapa_06_2_corrigir_ifnull_values_postgres.json"
    md_path = REPORTS_DIR / "etapa_06_2_corrigir_ifnull_values_postgres.md"

    salvar_json(json_path, relatorio)
    gravar_texto(md_path, gerar_markdown_relatorio(relatorio))

    print("Etapa 06.2 concluida.")
