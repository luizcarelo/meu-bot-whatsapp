// ============================================
// Arquivo: controllers/CrmController.js
// Descrição: Controlador do CRM
...
class CrmController {
    ...
    async getMensagens(req, res) {
        ...
        try {
            ...
            console.error('[CRM] Erro ao buscar mensagens:', e);
    ...
    async createContato(req, res) {
        ...
        try {
            const [existe] = await this.db.execute(
                'SELECT id FROM contatos WHERE empresa_id = ? AND telefone = ?',
                [req.empresaId, remoteJid]
            );
            ...
            console.error('[CRM] Erro ao criar contato:', e);
    ...
    async getContatos(req, res) {
        ...
        try {
            ...
            if (statusFiltro === 'meus') {
                ...
                if (isAdmin) {
                    ...
                    sql += ` AND (
                        ...
                        OR (c.status_atendimento = 'ATENDENDO' AND c.atendente_id = ?)
                    )`;
            ...
            console.error('[CRM] Erro ao buscar contatos:', e);
    ...
    async updateContato(req, res) {
        ...
        try {
            await this.db.execute(
                `UPDATE contatos SET nome=?, cnpj_cpf=?, email=?, endereco=?, anotacoes=? WHERE empresa_id=? AND telefone=?`,
                [nome, cnpj, email, endereco, anotacoes, req.empresaId, telefone]
            );
            this.emitirAtualizacao(req.empresaId, { action: 'update_contato', telefone });
            res.json({ success: true });
        } catch (e) {
            console.error('[CRM] Erro ao atualizar contato:', e);
            res.status(500).json({ error: e.message });
        }
    }

    /**
     * Obtém agenda (todos os contatos)
     * GET /api/crm/agenda
     */
    async getAgenda(req, res) {
        try {
            ...
            console.error('[CRM] Erro ao carregar agenda:', e);
    ...
    /**
     * Assumir atendimento de um contato
    ...
    async assumirAtendimento(req, res) {
        ...
        try {
            ...
            console.error('[CRM] Erro ao assumir:', e);
            res.status(500).json({ error: e.message });
        }
    }

    /**
     * Transferir atendimento para setor
     * POST /api/crm/atendimento/transferir
     */
    async transferirAtendimento(req, res) {
        ...
        try {
            ...
            await this.db.execute(
                ...
                [req.empresaId, telefone, `🔄 *Transferido para setor:* ${setor[0]?.nome}`]
            );

            this.emitirAtualizacao(req.empresaId, { action: 'transferir', telefone });
            res.json({ success: true });
        } catch (e) {
            console.error('[CRM] Erro ao transferir:', e);
            res.status(500).json({ error: e.message });
        }
    }
     ...
     * POST /api/crm/atendimento/transferir-usuario
     */
    async transferirParaUsuario(req, res) {
        ...
        try {
            ...
            await this.db.execute(
                `INSERT INTO mensagens (empresa_id, remote_jid, from_me, tipo, conteudo) VALUES (?, ?, 1, 'texto', ?)`,
                [req.empresaId, telefone, `🔄 *Transferido para usuário:* ${nomeUser}`]
            );

            this.emitirAtualizacao(req.empresaId, { action: 'transferir_user', telefone });
            res.json({ success: true });
        } catch (e) {
            console.error('[CRM] Erro ao transferir para usuário:', e);
            res.status(500).json({ error: e.message });
        }
    }
     ...
     * POST /api/crm/atendimento/encerrar
     */
    ...
    async getConfig(req, res) {
        try {
            const [rows] = await this.db.execute(
                `SELECT nome, nome_sistema, logo_url, cor_primaria, mensagens_padrao, 
                        msg_ausencia, msg_avaliacao, horario_inicio, horario_fim, dias_funcionamento, 
                        welcome_media_url, welcome_media_type, whatsapp_numero, whatsapp_status, 
                        whatsapp_updated_at, openai_key, openai_prompt, openai_ativo 
            ...
            console.error('[CRM] Erro ao buscar config:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async updateConfig(req, res) {
        ...
        try {
            ...
            console.error('[CRM] Erro ao atualizar config:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async updateConfigIA(req, res) {
        ...
        try {
            ...
            console.error('[CRM] Erro ao atualizar IA:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async getClientDashboard(req, res) {
        try {
            ...
            console.error('[CRM] Erro no dashboard:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async getAvaliacoes(req, res) {
        try {
            ...
            console.error('[CRM] Erro ao buscar avaliações:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async sendBroadcast(req, res) {
        ...
        try {
            ...
            for (const c of contatos) {
                ...
                await this.db.execute(
                    `INSERT INTO mensagens (empresa_id, remote_jid, from_me, tipo, conteudo) VALUES (?, ?, 1, 'texto', ?)`,
                    [req.empresaId, c.telefone, mensagem]
                );
                enviados++;
            }
            ...
            console.error('[CRM] Erro no broadcast:', e);
            res.status(500).json({ error: e.message });
        }
    }
    ...
    async createAtendente(req, res) {
        ...
        if (qtd[0].total >= emp[0].limite_usuarios) {
